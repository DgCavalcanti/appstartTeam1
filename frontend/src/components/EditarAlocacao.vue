...
async function salvar() {
  ...
  const resp = await store.ajustarAlocacao({
    alocacao_id:  alocacaoAtual.value.id,
    nova_sala_id: novaSalaId.value,
    justificativa: justificativa.value.trim() || undefined,
  });

  if (resp) {
    toast.success('Alocação atualizada. Conflitos recalculados.');
    emit('salvo');
  } else {
    toast.error(store.erro ?? 'Erro ao salvar alocação.');
  }
}
</script>
