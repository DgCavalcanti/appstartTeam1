    ...
    <aside :class="{ '-translate-x-full': !sidebarOpen }" class="bg-paper-sidebar text-gray-100 w-64 space-y-2 py-7 px-2 absolute inset-y-0 left-0 transform md:relative md:translate-x-0 transition duration-200 ease-in-out z-20 h-full shrink-0 overflow-y-auto">
      ...
      <nav class="space-y-1">
        ...
        <router-link to="/" class="flex items-center space-x-2 py-2.5 px-4 rounded transition duration-200 hover:bg-paper-active-link hover:text-white">
          <HomeIcon class="h-5 w-5"/><span>Início</span>
        </router-link>
        ...
        <!-- ── AGHU: Dados reais ─────────────────────────────────── -->
        <div class="px-4 pt-4 pb-1">
          <p class="text-xs font-semibold uppercase text-gray-400 tracking-wider">AGHU — Dados Reais</p>
        </div>

        <router-link to="/saa/capacidade" class="flex items-center space-x-2 py-2.5 px-4 rounded transition duration-200 hover:bg-paper-active-link hover:text-white">
          <ChartBarSquareIcon class="h-5 w-5"/><span>Capacidade</span>
        </router-link>

        <router-link to="/saa/consultas" class="flex items-center space-x-2 py-2.5 px-4 rounded transition duration-200 hover:bg-paper-active-link hover:text-white">
          <TableCellsIcon class="h-5 w-5"/><span>Consultas</span>
        </router-link>

        <router-link to="/saa/qualidade-dados" class="flex items-center space-x-2 py-2.5 px-4 rounded transition duration-200 hover:bg-paper-active-link hover:text-white">
          <ShieldCheckIcon class="h-5 w-5"/><span>Qualidade</span>
        </router-link>
        ...
        <router-link v-if="authStore.isAdmin" to="/admin" class="flex items-center space-x-2 py-2.5 px-4 rounded transition duration-200 hover:bg-paper-active-link hover:text-white">
          <ShieldCheckIcon class="h-5 w-5"/><span>Administração</span>
        </router-link>
      </nav>
    </aside>
...
import {
  ...
  ChartBarSquareIcon, TableCellsIcon,
