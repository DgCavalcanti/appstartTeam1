...
class MockAuthProvider(AuthProviderInterface):
    ...
    def authenticate_user(self, username, password) -> dict:
        ...
        if username == "admin" and password == "admin":
            logger.info("Authentication successful for mock user: %s", username)
            ...
            logger.warning("Authentication failed for mock user: %s", username)
            raise HTTPException(
...
class ActiveDirectoryAuthProvider(AuthProviderInterface):
    ...
    def authenticate_user(self, username, password) -> dict:
        logger.info("Starting AD Authentication (ldap3) for user: %s", username)
        try:
            ...
            logger.info("AD Authentication successful for user: %s", username)
            return user_info
...
class AuthHandler:
    def __init__(self):
        ...
        if os.getenv("AD_URL"):
            logger.info("Using Active Directory authentication.")
            self.provider: AuthProviderInterface = ActiveDirectoryAuthProvider()
        else:
            logger.warning("AD environment variables not found. Using Mock authentication.")
