...
# Nome do item de equipamento exigido para ginecologia (lista Sala.equipamentos)
EQUIPAMENTO_MACA_GINECOLOGICA = "maca_ginecologica"
...
def _bloqueada_por_ocupacao(sala: Sala, salas_ocupadas: set[str]) -> bool:
...
def calcular_score(
    ...
    salas_do_historico_do_medico: set[str],
) -> int:
    """
...
def alocar(
    ...
    historico_por_profissional: dict[str, set[str]] = {}
    for aloc in historico:
        grade_ref = next((g for g in grades if g.id == aloc.grade_id), None)
        if grade_ref:
            historico_por_profissional.setdefault(grade_ref.profissional, set()).add(aloc.sala_id)

    salas_ocupadas: set[str] = set()
    ...
    for grade in grades_ordenadas:
        logger.debug("Processando grade id=%s esp=%s prof=%s qtd=%d",
        ...
        if not candidatas:
            logger.warning("Grade id=%s sem salas candidatas após filtro bloqueante", grade.id)
        ...
        if len(salas_escolhidas) < qtd:
            logger.warning(
                "Grade id=%s precisava de %d salas, conseguiu apenas %d",
